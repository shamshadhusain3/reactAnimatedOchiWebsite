...
function Navbar() {
  const [toggleNavBg, settoggleNavBg] = useState(false);
  useEffect(() => {
    window.addEventListener('scroll', () => {
      if (window.scrollY > 900) {
        settoggleNavBg(true);
      } else {
        settoggleNavBg(false);
      }
    });
...
// ["Services", "Our Work","About me","Insights","Contact"]
  return (
    <div  className={`fixed z-[1999] w-full px-20 py-8 font-["Neue Montreal"] flex justify-between items-center
    ${toggleNavBg? 'bg-gray-400':'bg-[none]'} rounded-md bg-clip-padding backdrop-filter backdrop-blur-lg bg-opacity-10
    `}>
        <div className="logo">
        ...
        <div className="links flex gap-5">
             {
                menus.map((item,index)=>(
                    <a href={item.href} className={`text-md capitalize font-light ${index===4 && 'ml-32'}`} key={index}>{item.title}</a>
                ))
             }
        </div>
    </div>
